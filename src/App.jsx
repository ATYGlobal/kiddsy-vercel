 ...
 *   - Logo is now inline in Navbar only — no separate component needed
 *
 * NEW SECTIONS:
 *   - /wordsearch  → Word Search game (sopa de letras)
 *   - /animals     → Animal Puzzle (real photos, drag-to-swap tiles)
 *
...
function StarField() {
  const stars = Array.from({ length: 14 }, (_, i) => ({
    ...
    duration:Math.random()*3+3, opacity:Math.random()*0.25+0.07,
  }));
  return (
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-md border-b-2 border-white shadow-sm">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-2">
        ...
        {/* ── Logo (inline — no separate KiddsyLogo component) ── */}
        <button onClick={() => { onNav("library"); setMenuOpen(false); }}
          className="flex items-center gap-2.5 flex-shrink-0"
        >
          <motion.div
            whileHover={{ rotate:[-3,3,0], scale:1.08 }} transition={{ duration:0.4 }}
            className="w-11 h-11 rounded-2xl overflow-hidden shadow-md border-2 flex items-center justify-center"
      ...
      <div className="bg-white/90 backdrop-blur-md rounded-4xl shadow-xl border-4 border-white p-8 md:p-10">
        ...
        <div className="space-y-5">
          ...
          <motion.button whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }}
            ...
            style={{
              background:childName.trim()&&theme&&!loading?`linear-gradient(135deg,${C.blue},#42A5F5)`:"#E5E7EB",
              color:childName.trim()&&theme&&!loading?"white":"#9CA3AF",
              cursor:childName.trim()&&theme&&!loading?"pointer":"not-allowed",
            }}
          >{loading ? "✨ Writing the story…" : "🪄 Generate Story"}</motion.button>
        </div>
      </div>
    </motion.div>
  );
    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}>
      ...
      {stories.length === 0 ? (
        <div className="text-center py-16 text-slate-400 font-body">
          ...
          <p className="text-xs mt-1">Make sure VITE_SUPABASE_URL and API are configured.</p>
        </div>
      ) : (
  ...
  return (
    <div className="min-h-screen relative" style={{ background:"#FFFDE7" }}>
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full blur-3xl" style={{ background:`${C.blue}08` }}/>
        <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full blur-3xl" style={{ background:`${C.yellow}10` }}/>
      </div>

      <div className="relative z-10">
        <Navbar view={view} onNav={handleNav}/>

        {/* Language picker for story views */}
